...
window.onload = function() {
  ...
  else if(event.keyCode === 65 || event.keyCode === 87 || event.keyCode === 68 || event.keyCode === 83){
    direction2 = getDirection2(event)
  }
  }, false);
...
requestAnimationFrame(function gameLoop() {
  ...
  if (_.find(wall, [block2.x, block2.y]) === undefined ) {
    // for(var i = 0; i < 6; i++) {
    wall.push([block2.x, block2.y]);
    // }
  }
  ...
  if ( collisionCheck(block)){
    ...
    return context.fillText("Player 1 Wins!", 120, 190);
    // console.log('Player 1 Wins!');
  }
  else if(collisionCheck(block2)){
    context.font = '60px tron';
    context.fillStyle = '#FF1818';
    return context.fillText("Player 2 Wins!", 120, 190);
  }
